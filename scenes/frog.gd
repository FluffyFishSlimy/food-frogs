...
@onready var mouth_open: Panel = $mouth_open
@onready var mouth_smile: Line2D = $mouth_smile
...
func _on_shoot_timer_timeout() -> void:
	if shooting and enemies_in_range.is_empty() == false and fruit_eaten != null:
		for spawner in bullet_spawners:
			...
			SoundManager.play_sound("button_press", randf_range(0.8, 1.2), -15)
			mouth_open.show()
			mouth_smile.hide()
...
func _on_click_detect_pressed() -> void:
	var t = create_tween().set_trans(Tween.TRANS_CIRC)
	if randi_range(0,1) == 0:
		SoundManager.play_sound('squeak', randf_range(0.8, 1.2), 0)
	else:
		SoundManager.play_sound('squeak2', randf_range(0.8, 1.2), 0)
	if randi_range(0,1) == 0:
