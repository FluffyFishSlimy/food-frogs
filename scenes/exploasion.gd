...
var bullet_damge:float = 0.75

func _ready() -> void:
	scale = Vector2.ZERO
	rotation_degrees = randi_range(0, 360)
	var t = create_tween().set_trans(Tween.TRANS_CIRC)
	t.tween_property(self, 'scale', Vector2(randf_range(0.8, 1.2), randf_range(0.8, 1.2)), 0.2)
	SoundManager.play_sound('exploasion', randf_range(0.9, 1.1), -10)
